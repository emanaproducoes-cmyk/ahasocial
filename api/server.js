...
const db=admin.firestore(),TS=admin.firestore.FieldValue.serverTimestamp;
async function auth(req,res,next){const t=req.headers.authorization?.split('Bearer ')[1];if(!t)return res.status(401).json({error:'Token não fornecido.'});try{req.user=await admin.auth().verifyIdToken(t);next();}catch{res.status(401).json({error:'Token inválido.'});}}
app.get('/health',(_,res)=>res.json({status:'ok',time:new Date()}));
// Posts
app.get('/api/posts',auth,async(_,res)=>{const s=await db.collection('posts').orderBy('createdAt','desc').get();res.json(s.docs.map(d=>({id:d.id,...d.data()})));});
app.post('/api/posts',auth,async(req,res)=>{const r=await db.collection('posts').add({...req.body,createdBy:req.user.uid,createdAt:TS()});res.status(201).json({id:r.id});});
app.patch('/api/posts/:id',auth,async(req,res)=>{await db.collection('posts').doc(req.params.id).update({...req.body,updatedAt:TS()});res.json({id:req.params.id});});
app.delete('/api/posts/:id',auth,async(req,res)=>{await db.collection('posts').doc(req.params.id).delete();res.json({deleted:true});});
// Accounts
app.get('/api/accounts',auth,async(_,res)=>{const s=await db.collection('accounts').orderBy('createdAt','desc').get();res.json(s.docs.map(d=>({id:d.id,...d.data()})));});
app.post('/api/accounts',auth,async(req,res)=>{const r=await db.collection('accounts').add({...req.body,createdBy:req.user.uid,createdAt:TS()});res.status(201).json({id:r.id});});
app.patch('/api/accounts/:id',auth,async(req,res)=>{await db.collection('accounts').doc(req.params.id).update({...req.body,updatedAt:TS()});res.json({id:req.params.id});});
app.delete('/api/accounts/:id',auth,async(req,res)=>{await db.collection('accounts').doc(req.params.id).delete();res.json({deleted:true});});
// Campaigns
app.get('/api/campaigns',auth,async(_,res)=>{const s=await db.collection('campaigns').orderBy('createdAt','desc').get();res.json(s.docs.map(d=>({id:d.id,...d.data()})));});
app.post('/api/campaigns',auth,async(req,res)=>{const r=await db.collection('campaigns').add({...req.body,status:'active',posts:0,approved:0,pending:0,rejected:0,createdBy:req.user.uid,createdAt:TS()});res.status(201).json({id:r.id});});
app.patch('/api/campaigns/:id',auth,async(req,res)=>{await db.collection('campaigns').doc(req.params.id).update({...req.body,updatedAt:TS()});res.json({id:req.params.id});});
app.delete('/api/campaigns/:id',auth,async(req,res)=>{await db.collection('campaigns').doc(req.params.id).delete();res.json({deleted:true});});
// Aprovação pública
app.get('/api/approval/:id',async(req,res)=>{const d=await db.collection('posts').doc(req.params.id).get();if(!d.exists)return res.status(404).json({error:'Não encontrado.'});const p={id:d.id,...d.data()};res.json({id:p.id,title:p.title,platform:p.platform,date:p.date,caption:p.caption,status:p.status,campaign:p.campaign,fileUrl:p.fileUrl,fileType:p.fileType});});
app.post('/api/approval/:id',async(req,res)=>{const{action,comment,clientName}=req.body;const sm={approve:'approved',reject:'rejected',correct:'pending'};const ns=sm[action];if(!ns)return res.status(400).json({error:'Ação inválida.'});await db.collection('posts').doc(req.params.id).update({status:ns,clientReview:{action,comment,clientName,reviewedAt:TS()}});res.json({success:true,newStatus:ns});});
// Instagram token exchange
app.post('/api/instagram/token',auth,async(req,res)=>{
...
// Instagram Insights
app.get('/api/instagram/insights/:accountId',auth,async(req,res)=>{
